...
describe('Logger Module', function() {
    ...
    let logBuildResults: (results: { webClient?: { path: string; size: number }; webServer?: { path: string; size: number }; events?: { path: string; size: number } }) => void;
    ...
    describe('logDevServerBanner', function() {
        it('should log banner with all URLs', function() {
            ...
            const allOutput = consoleLogStub.getCalls().map(call => call.args[0]).join('\n');
